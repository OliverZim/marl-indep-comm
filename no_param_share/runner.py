    ...
    def run(self, num):
        ...
        while time_steps < self.args.n_steps:
            if time_steps // self.args.evaluate_cycle > evaluate_steps:
                ...
                plt.savefig(
                    self.save_path + "/plt_{}_{}_{}ts.png".format(num, self.args.env, self.args.n_steps),
                    format="png",
                )
                np.save(
                    self.save_path + "/episode_rewards_{}_{}_{}ts".format(num, self.args.env, self.args.n_steps),
                    episode_rewards,
                )
                np.save(
                    self.save_path + "/win_rates_{}_{}_{}ts".format(num, self.args.env, self.args.n_steps),
                    win_rates,
                )
                ...
                for train_step in range(self.args.train_steps):
                    ...
                    if self.args.with_comm:
                        for agent_id in range(self.args.n_agents):
                            msgs_i, msgs_i_next = self.list_agents[agent_id].get_msgs_for_train(
                                mini_batch, train_steps, agent_id
                            )
