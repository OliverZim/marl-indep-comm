...
class IDQL:
    ...
    def learn(self, batch, max_episode_len, train_step, agent_id, epsilon=None, all_msgs=None, all_msgs_next=None):
        ...
        obs, obs_next, actions, reward, avail_actions, avail_actions_next, terminated = (
            batch["obs"][:, :, agent_id],
        ...
        for transition_idx in range(max_episode_len):
            ...
            if self.args.with_comm:
                ...
                q_eval, self.eval_hidden = self.eval_rnn(
                    inputs, self.eval_hidden, msgs=others_msgs_tidx, agent_num=agent_id, msg_i=msg_agent_tidx
                )
                q_target, self.target_hidden = self.target_rnn(
                    inputs_next, self.target_hidden, msgs=others_msgs_next_tidx, agent_num=agent_id, msg_i=msg_agent_next_tidx
                )
            else:
