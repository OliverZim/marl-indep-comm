...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.buffers = {
            "obs": np.empty([self.size, self.episode_limit, self.n_agents, self.obs_shape]),
            "actions": np.empty([self.size, self.episode_limit, self.n_agents, 1]),
            "state": np.empty(
                [self.size, self.episode_limit, self.state_shape]
            ),  # state is global regardless the agent; this is the diff between state and observation
            ...
            "padded": np.empty(
                [self.size, self.episode_limit, 1]
            ),  # note: padded is to pad the episode buffer if the episode generated length is less than episode_limit
